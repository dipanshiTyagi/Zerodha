...
app.post('/newOrder', async(req, res) => {
    try {
        ...
        if (holding) {
            // If holding exists, update the qty
            holding.qty += (mode === 'buy' ? qty : -qty); // Increase for buy, decrease for sell
            ...
            holding = new HoldingsModel({
                name,
                qty,
                avg: price // Initial average cost
            });
        }

        await holding.save();

        res.json({ newOrder, holding }); 
    } catch (error) {
